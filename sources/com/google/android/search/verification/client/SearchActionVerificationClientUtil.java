...
public abstract class SearchActionVerificationClientUtil {
    ...
    public static Uri getAudioContentUri(Notification notification) {
        return getAudioContentUriFromBundle(C002701i.A0E(notification));
    }
    ...
    public static Integer getContentId(Notification notification) {
        return getContentIdFromBundle(C002701i.A0E(notification));
    }
    ...
    public static boolean getRequiresAudioInput(C12830k9 remoteInput) {
        return remoteInput.A00.getBoolean(INPUT_REQUIRES_AUDIO_INPUT_KEY);
    }
    ...
    public static boolean isPackageGoogleSigned(Context context, String pkg) {
        try {
            ...
            if (signatureArr != null && signatureArr.length == 1) {
                ...
                Log.d(TAG, String.format("Package %s GSA signed status %s AssistantGo signed status %s", pkg, Boolean.valueOf(equals), Boolean.valueOf(equals2)));
    ...
    public static C005402l setAudioContentUri(C005402l builder, Uri audioContentUri) {
        if (audioContentUri == null) {
            builder.A02().remove(AUDIO_CONTENT_URI_KEY);
            return builder;
        }
        builder.A02().putString(AUDIO_CONTENT_URI_KEY, audioContentUri.toString());
        return builder;
    }

    public static C005402l setContentId(C005402l builder, Integer id) {
        if (id == null) {
            builder.A02().remove(CONTENT_ID_KEY);
            return builder;
        }
        builder.A02().putInt(CONTENT_ID_KEY, id.intValue());
        return builder;
    }

    public static C12820k8 setRequiresAudioInput(C12820k8 builder, boolean requiresAudioInput) {
        throw null;
    }
}
